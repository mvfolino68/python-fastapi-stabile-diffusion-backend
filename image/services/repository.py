...
async def retrieve_document(document_id: str, collection: str) -> dict:
    ...
    if document := await image.app.state.mongo_collection[collection].find_one(document_filter):
...
async def create_document(document: dict, collection: str) -> dict:
    ...
    """
    try:
        document = await image.app.state.mongo_collection[collection].insert_one(document)
